 ...
 *
 */
public class JQueryVersionPage extends AbstractWizardPageWithPreview implements IPreferredJSLibVersion, JQueryConstants {
	...
	void reloadFields() {
		...
		getWizard().getPreferredVersions().applyLibPreference(this);
		...
		checkboxIds.clear();
		selectorIds.clear();
		getWizard().getPreferredVersions().updateLibEnablementAndSelection();
	...
	void addContent(ElementNode root) {
		...
		for (JSLib lib: libs) {
			String libName = lib.getName();
			if(getWizard().getPreferredVersions().isLibDisabled(libName)) continue;
