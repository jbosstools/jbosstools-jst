 ...
 *
 */
public class JQueryMobileRecognizer implements ITagLibRecognizer {
	...
	public boolean shouldBeLoaded(ITagLibrary lib, ELContext context) {
		...
		if(resource instanceof IFile) {
			...
			if(FileUtil.isDoctypeHTML(file)) {
				lastResult = containsJQM(file);
			}
		}
		return lastResult;
	}

	public static boolean containsJQM(IFile file) {
				...
				if(htmlNode!=null) {
					...
					if(headNode!=null) {
						...
						for (Element script : scriptNodes) {
							if(Pattern.matches(JQUERY_MOBILE_JS_PATTERN, getAttribute(script, "src"))) {
								return true;
							}
						}
					}
