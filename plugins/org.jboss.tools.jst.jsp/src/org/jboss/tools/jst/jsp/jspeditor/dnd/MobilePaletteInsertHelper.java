...
public class MobilePaletteInsertHelper extends PaletteInsertHelper {
	...
	private void copyAttributes(IDOMNode node){
		...
		for(int i = 0; i < attributes.getLength(); i++){
			...
			globalBuffer.append(" "+attribute.getNodeName()+"=\""+attribute.getNodeValue()+"\"");
		}
	}
	...
	private void insertNode(IDocument document, IDOMNode relatedNode, IDOMNode node, String spaces, String text, int mode, boolean forceWrite) throws BadLocationException{
		...
		if(copyAttributes){
			copyAttributes(node);
			if(htmlNode != null && htmlNode.getNodeName().equals(node.getNodeName())){
				htmlNode = null;
			}else if(headNode != null && headNode.getNodeName().equals(node.getNodeName())){
				headNode = null;
			}else if(bodyNode != null && bodyNode.getNodeName().equals(node.getNodeName())){
				bodyNode = null;
			}
		}
