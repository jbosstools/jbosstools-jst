...
import org.jboss.tools.jst.jsp.format.HTMLAnchorProcessor;
...
//		int p1i = string.indexOf(':');
	...
	public IInformationControlCreator getInformationControlCreator() {
		Shell shell= JspEditorPlugin.getActiveWorkbenchShell();
		if (shell == null || !BrowserInformationControl.isAvailable(shell))
			return null;

		if (fCreator == null) {
			PresenterControlCreator presenterControlCreator= new PresenterControlCreator(JspEditorPlugin.getSite());
	...
	protected String getCSSStyles() {
		if (fCSSStyles == null) {
			Bundle bundle= Platform.getBundle(JspEditorPlugin.PLUGIN_ID);
